...
export async function nextRunningNo(tx: Tx, type: RunningType): Promise<string> {
  ...
  // lock the row for this type+year (or the type's single row for non-yearly)
  ...
  if (!row && def.yearly) {
    ...
    const [ins] = await tx
      ...
      .values({
        runningType: type,
        companyId: p?.company_id ?? 0,
        branchId: p?.branch_id ?? 0,
        prefix: p?.prefix ?? def.prefix,
        pyear: year,
        pmonth: 0,
        pday: 0,
        number: 0,
    ...
    const [ins] = await tx
      ...
      .returning({ id: runningNo.runningId });
    row = { running_id: ins.id, prefix: def.prefix, pyear: 0, number: 0, length_year: 0, length_number: def.len };
  }
