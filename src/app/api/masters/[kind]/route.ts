...
export const POST = handle(async (req: NextRequest, ctx: { params: Promise<{ kind: string }> }) => {
  ...
  if (kind === "symptoms") {
    const row = await saveSymptom({ id: s("id"), name: s("name") ?? "", detail: s("detail"), group: s("group"), status: s("status") }, me.userId);
    return NextResponse.json({ ok: true, row });
  }
  if (kind === "models") {
    const row = await saveModel({ code: s("code"), name: s("name") ?? "", brand: s("brand") ?? "", price: s("price"), status: s("status") }, me.userId);
    return NextResponse.json({ ok: true, row });
  }
