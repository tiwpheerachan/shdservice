...
- `/api/track` (POST เท่านั้น) rate limit 20/นาที + ล็อกต่อเลข 15 นาทีหลังผิด 5 ครั้ง + Turnstile หลังผิด 2 ครั้ง (ข้ามได้ถ้าไม่ตั้ง key) · สำเร็จแล้วคืน `/t/<token>` ให้เบราว์เซอร์ไปหน้าเดียวกับ QR
...
- Cloudflare (ยังไม่ได้ตั้ง — ทำทีหลัง): rate limiting rule `/t/` + `/api/track`, WAF challenge นอกไทย, cache bypass, Turnstile widget · **Render Subdomain ปิดแล้ว** ทุก traffic ผ่าน Cloudflare
