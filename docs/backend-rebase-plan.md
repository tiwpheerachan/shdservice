...
- **ข้อตกลง** (แก้ 2026-09-23 ระหว่างวัน): ลูกค้าติดตามได้**ตั้งแต่เปิดงาน** (`QUOTATION_REQUIRED = false`) — เจ้าหน้าที่คัดลอกลิงก์จากกล่องที่ขึ้นหลังบันทึกเปิดงานแล้วส่งทาง LINE · เดิมกำหนดว่าต้องมีใบเสนอราคาก่อน (ใบเสนอราคาเป็นเอกสารใบเดียวที่ลูกค้าได้รับจริง — ใบรับงานซ่อมเป็นเอกสารภายใน) · QR ยังพิมพ์บน**ใบเสนอราคาใบเดียว** · ไม่แสดงราคาใด ๆ บนหน้าติดตาม
...
## 36. โปรไฟล์บริษัทขนส่ง + ปุ่มติดตามพัสดุในหน้าลูกค้า (2026-09-23)
- **ปัญหาที่พบในข้อมูลจริง**: ระบบเก่าไม่เคยเก็บว่า *ขาส่งคืน* ใช้ขนส่งเจ้าไหน — `return_customer_type` เก็บแค่วิธี (บริษัทขนส่งเอกชน 40,109 / ลูกค้ามารับเอง 3,322) ส่วนชื่อเจ้าถูกยัดปนกับหมายเหตุใน `return_customer_detail` แบบ free text (`Flash` 10,354 · `FLASH` 7,554 · `J&T` 8,892 · `SHOPEE EXPRESS` 4,963 · ปนคำที่ไม่ใช่ชื่อขนส่ง เช่น `ระบบล่ม`) · เลขพัสดุมี 39,929 ใบ (ขึ้นต้น `TH…` 29,508)
- **ตัดสินใจ**: ทำ `shipping_profile` (drizzle/0013) แบบเดียวกับโปรไฟล์ผู้ออกเอกสาร — code / ชื่อ / โลโก้ (bucket `shippers/<id>/`) / `track_url` ที่มี `{no}` / active / sort / soft delete · seed Flash, J&T, SPX, KEX, ไปรษณีย์ไทย (แก้ URL ได้ในหน้า admin เพราะขนส่งเปลี่ยน URL บ่อย)
- `job.return_shipper_id` บันทึกตอนปิดงาน · หน้าปิดงานเปลี่ยนช่อง "บริษัทขนส่ง" เป็น dropdown + ตัวเลือก "อื่น ๆ (พิมพ์เอง)" ซึ่งยังเขียนข้อความลง `return_customer_detail` เหมือนเดิม
- **งานเก่าไม่ย้อนแก้** (ตามที่ตกลง) — ไม่มี `return_shipper_id` = ลูกค้าเห็นเลขพัสดุเฉย ๆ ไม่มีโลโก้/ปุ่ม
- หน้า `/track/<token>` ขั้น "ส่งคืน/รอรับเครื่อง" แสดงโลโก้ + เลขพัสดุ + ปุ่ม **ติดตามพัสดุ** (`rel="noopener noreferrer nofollow"`, target=_blank)
- เมนูใหม่ **ข้อมูลระบบ → โปรไฟล์บริษัทขนส่ง** (System Admin) · upload kind ใหม่ `shipper-logo`
